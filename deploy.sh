...
push_with_retry() {
  ...
  for attempt in $(seq 1 $max); do
    info "Pushing $image (attempt $attempt/$max)..."
    docker push "$image" && return 0
    info "Push failed — waiting 10s before retry..."
    sleep 10
  done
  error "Failed to push $image after $max attempts."
}
...
[ -f .env ] || error ".env file not found. It must exist at the project root."
info "Loading credentials from .env..."
# Parse .env line by line so values with spaces are handled correctly
...
# Warn about empty SMTP — deployment will succeed but emails won't send
if [ -z "${SMTP_USERNAME:-}" ] || [ -z "${SMTP_PASSWORD:-}" ]; then
  echo "[WARN]  SMTP_USERNAME or SMTP_PASSWORD is empty in .env — email notifications will not work."
  echo "        Update .env and re-run if you need email for the demo."
fi

# Regenerate JWT_SECRET if it looks like the dev placeholder
JWT_SECRET_LEN=${#JWT_SECRET}
if [ "$JWT_SECRET_LEN" -lt 64 ]; then
  info "JWT_SECRET in .env is too short for production ($JWT_SECRET_LEN chars). Generating a secure one..."
  JWT_SECRET=$(openssl rand -base64 48)
  info "Generated JWT_SECRET (save this if you need to reuse it across deployments):"
...
# Build JARs locally first — faster and produces smaller Docker images
info "Building JARs with Maven..."
for dir in identityService inventoryService analyticsService notificationService; do
  info "  mvn package: $dir"
...
# ─── Step 7: Secrets (built from .env — never committed to git) ──────────────
info "Creating k8s secret from .env values..."
kubectl create secret generic inventalert-secrets \
  --namespace "$NAMESPACE" \
  --from-literal=JWT_SECRET="$JWT_SECRET" \
  --from-literal=MYSQL_ROOT_PASSWORD="${MYSQL_ROOT_PASSWORD}" \
  --from-literal=MYSQL_DATABASE="${MYSQL_DATABASE}" \
  --from-literal=MYSQL_IDENTITY_USER="${MYSQL_IDENTITY_USER}" \
  --from-literal=MYSQL_IDENTITY_PASSWORD="${MYSQL_IDENTITY_PASSWORD}" \
  --from-literal=MYSQL_INVENTORY_USER="${MYSQL_INVENTORY_USER}" \
  --from-literal=MYSQL_INVENTORY_PASSWORD="${MYSQL_INVENTORY_PASSWORD}" \
  ...
  --from-literal=SUPER_ADMIN_EMAIL="${SUPER_ADMIN_EMAIL}" \
  --from-literal=SUPER_ADMIN_PASSWORD="${SUPER_ADMIN_PASSWORD}" \
  --from-literal=SUPER_ADMIN_ID="${SUPER_ADMIN_ID}" \
  --from-literal=NOTIFICATION_TTL_DAYS="${NOTIFICATION_TTL_DAYS:-30}" \
...
for deploy in identity-service inventory-service analytics-service notification-service nginx; do
  kubectl rollout status deployment/"$deploy" -n "$NAMESPACE" --timeout=420s
done
...
# Patch CORS now that the real frontend URL is known
info "Patching CORS_ALLOWED_ORIGINS to $FRONTEND_URL ..."
kubectl create secret generic inventalert-secrets \
  --namespace "$NAMESPACE" \
  --from-literal=JWT_SECRET="$JWT_SECRET" \
  --from-literal=MYSQL_ROOT_PASSWORD="${MYSQL_ROOT_PASSWORD}" \
  ...
  --from-literal=SUPER_ADMIN_ID="${SUPER_ADMIN_ID}" \
  --from-literal=NOTIFICATION_TTL_DAYS="${NOTIFICATION_TTL_DAYS:-30}" \
  --from-literal=CORS_ALLOWED_ORIGINS="${FRONTEND_URL}" \
  --from-literal=GOOGLE_MAPS_API_KEY="${GOOGLE_MAPS_API_KEY:-}" \
  --dry-run=client -o yaml | kubectl apply -f -

info "Restarting deployments to pick up updated CORS..."
